...
import type { AddonWithCategoryOption, OrderItemAddon } from '@/types'
...
export function AddonSelectorModal() {
  ...
  const addonsToSubmit = useMemo((): OrderItemAddon[] => {
    ...
    for (const { addon, includedFree, extraPrice } of options) {
      ...
      if (qty > 0) {
        const price = extraPrice ?? addon.price
        list.push({
          addonId: addon.id,
          type: 'extra',
          name: addon.name,
    ...
    return list
  }, [addonModalProduct, options, includedAddons, extraQuantities])
  ...
  const calculateTotal = () =>
    (addonModalProduct?.price ?? 0) + totalAddonsPrice
  ...
  const handleAdd = () => {
    if (!addonModalProduct) return
    addItemWithAddons(addonModalProduct, addonsToSubmit)
    closeAddonModal()
  }

  const handleAddWithoutAddons = () => {
    if (!addonModalProduct) return
    addItemWithAddons(addonModalProduct, [])
    closeAddonModal()
  }

  if (!addonModalProduct) return null

  const productImage =
    addonModalProduct.image ||
    'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?q=80&w=400&auto=format&fit=crop'

  return (
    <BaseModal
      open={addonModalOpen}
      onOpenChange={(open) => !open && closeAddonModal()}
      title={`Ajouter: ${addonModalProduct.name}`}
      maxWidth="md"
      footer={
        ...
        <div className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg">
          ...
          <div className="flex-1 min-w-0">
            <h4 className="text-gray-900 font-medium truncate">
              {addonModalProduct.name}
            </h4>
            <p className="text-[#F4A024] font-bold">
              {formatPrice(addonModalProduct.price)}
            </p>
          </div>
        </div>
        ...
        {hasAnySelection && (
          <div className="pt-4 border-t border-gray-200">
            <div className="flex items-center justify-between text-sm text-gray-600 mb-1">
              <span>Produit</span>
              <span>{formatPrice(addonModalProduct.price)}</span>
            </div>
